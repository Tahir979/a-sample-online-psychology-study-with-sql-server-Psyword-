...
namespace DEMO
{
    public partial class loginscreen : Form
    {
        ...
        SqlConnection con = new SqlConnection("Server=tcp:78.135.105.180,1433;Initial Catalog=newpsyword;Persist Security Info=False;User ID=psyword;Password=HARDWAWERWARE1.;MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=True;Connection Timeout=30");
        SqlCommand cmd;
        ...
        void giris()
        {
            ...
            else
            {
                            ...
                            if (gridparticipants.Rows[0].Cells[0].Value.ToString() == username.Texts)
                            {
                                if (gridparticipants.Rows[0].Cells[0].Value.ToString() == username.Texts && gridparticipants.Rows[0].Cells[1].Value.ToString() == password.Texts)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        string mevcut = mevcut_x.Substring(0, 10); //28
                                        ...
                                        {
                                            mevcut_x = DateTime.Now.ToString("dd/MM/yyyy");
                                            string karakter_x = mevcut_x.Substring(2,1);
                                            mevcut = mevcut_x.Substring(0, 2); //28 
                                            int cevir_mevcut = Convert.ToInt32(mevcut);
                                            int eksilt = Convert.ToInt32(mevcut);
                                            int eksik = eksilt - 1;
                                            ...
                                            if (mevcut_ay == "12")
                                            {
                                                j = eksik + karakter_x + "12" + karakter_x + "2022";
                                            }
                                            else if (mevcut_ay == "01")
                                            {
                                                j = eksik + karakter_x + "01" + karakter_x + "2023";
                                            }

                                            if (eksik < 10)
                                            {
                                                j = "0" + eksik + karakter_x + "01" + karakter_x + "2023";
                                            }
                                            ...
                                            else if (hayir == 2 && girisPsyword.Rows[0].Cells[6].Value.ToString() != gunumuz)
                                            {
                                                pctscreen.Image = Properties.Resources.kullanici_elendi_min_min;
                                            ...
                                            else if (hayir == 1 && cevir + 1 == cevir_mevcut)
                                            {
                                                griddoldur();

                                                DataView dv6 = tablo2.DefaultView;
                                                dv6.RowFilter = "kayittamam Like '%" + "hayir" + "%' and kAdi like '%" + username.Texts + "%'";
                                                girisPsyword.DataSource = dv6;
                                                ...
                                                else
                                                {
                                                    ...
                                                    cmd.Parameters.AddWithValue("kAdi", username.Texts);
                                                    cmd.Parameters.AddWithValue("kGrup", sessiontext.Text);
                                                    cmd.Parameters.AddWithValue("oturum", girisPsyword.Rows.Count + 2);
                                                    cmd.Parameters.AddWithValue("girisDatetime", "");
                                                    cmd.Parameters.AddWithValue("cikisDatetime", "");
                                                    cmd.Parameters.AddWithValue("totalsureDatetime", "");
                                                    cmd.Parameters.AddWithValue("mevcutDatetime", DateTime.Now.ToString("dd/MM/yyyy"));
                ...
                {
                    MessageBox.Show("Sanırız server biraz sorunlu, lütfen tekrar deneyiniz ve internet bağlantınızın sabit olduğundan emin olunuz (kullanici, giris void 1 hata)");
                }
            }
        }
        void netkontrol()
        {
            try
            {
                ...
                if (reply.Status == IPStatus.Success)
                {
                    ...
                    da = new SqlDataAdapter("filterKatilimcilar", con);
                    tablo.Clear();
                    da.Fill(tablo);
        ...
        void griddoldur()
        {
            ...
            da = new SqlDataAdapter("filterGirisPsyword", con);
            tablo2.Clear();
            da.Fill(tablo2);
        ...
        private void kullanici_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                pnlclosetheprogram.Parent = pctscreen;
